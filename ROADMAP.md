...
| Sprint | O que era |
|---|---|
| S156 | Bug: teto de texto no ramo de EDITAR mensagem ficava abaixo do teto de CRIAR, nos dois chats — `firestore.rules`, `matches/{matchId}/messages` (S92) e `groups/{groupId}/messages` (S149-D, mirror exato do 1:1, herdou o mesmo erro): `allow update` tinha `text.size() <= 500`, mas `allow create` do mesmo bloco já usava `<= 2000` desde a S77, e o client (`MAX_MESSAGE_LENGTH`/`maxLength` em `ChatScreen.tsx`/`GroupChatScreen.tsx`) também já era 2000 — mensagem entre 501 e 2000 caracteres podia ser criada mas não editada (permission-denied silencioso). Corrigido nos dois blocos, teto de editar igualado a 2000. De caminho: carimbo do topo do `firestore.rules` ganhou entrada nova (S156) e a data final da "linha histórica" do mesmo carimbo, presa em 2026-08-03 mesmo com S149-B/S149-C já anexadas depois, foi corrigida pra 2026-08-26. `firestore.rules` **NÃO deployadas**. Sem Cloud Function, sem mudança de client. **Fechada em código, SEM teste em aparelho.** |
