...
// timeZone do padrão de staleMatchReminder. A transação por doc (em vez de
// deleteDocsInBatches direto na lista da query) é necessária porque a lista
// da query, coletada no passo 1, pode ficar desatualizada: um usuário que
// sobrescreve o próprio momento (setDoc) entre a query e o commit teria o
// NOVO momento apagado por engano, já que a query capturou o doc ANTIGO
// antes da sobrescrita. A releitura dentro da transação garante que só
// apaga o que ainda está realmente expirado no momento do delete.
...
// de expireGroups acima (evita apagar um doc que mudou entre a query e o
// commit) — diferença deliberada de schedule: 1x/dia (decisão 11 — a folga
// aceitável aqui é de DIAS, não de minutos, ao contrário de
...
// Mirror EXATO de onGroupJoinRequestCreated (S124-A) acima, pra
...
// produto. Mirror EXATO de onGroupMemberCreated acima.
